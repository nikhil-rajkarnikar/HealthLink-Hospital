 ...
 * @author user
 */
public class ButtonBillingCell<S> extends TableCell<S, Void> {
 private final Button billingButton = new Button("Billing");

    public ButtonBillingCell(TableColumn<S, Void> column) {
        ...
        billingButton.setOnAction(event -> {
            ...
            System.out.println("rowData =>>" + rowData);
             ...
             if (rowData instanceof Patient) {
                ...
                handleViewBilling(patient);
            }
        });
        

        HBox pane = new HBox(billingButton);
        pane.setSpacing(5);
        setGraphic(pane);
    }
    ...
    public void handleViewBilling(Patient patient) {
        try {
